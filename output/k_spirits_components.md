# [Project Context] k-spirits-club-hub - Module: components
...
### File: components\admin\AdminContent.tsx
```
'use client';
...
### File: components\admin\AdminSpiritCard.tsx
```
'use client';
...
### File: components\admin\DiscoveryLogsTable.tsx
```
'use client';
...
### File: components\admin\ModificationRequestsTable.tsx
```
'use client';
...
### File: components\auth\LoginClient.tsx
```
'use client';
...
### File: components\cabinet\AddSpiritCard.tsx
```
'use client';
...
### File: components\cabinet\CabinetClient.tsx
```
'use client';
...
### File: components\cabinet\CabinetSpiritCard.tsx
```
'use client';
        ...
        >
            ...
            <div className="relative aspect-2/3 rounded-2xl overflow-hidden bg-white dark:bg-slate-800/50 p-px shadow-sm transition-all duration-500 group-hover:shadow-amber-500/20">
                ...
                <div className="relative h-full rounded-xl overflow-hidden bg-slate-50 dark:bg-slate-900 group-hover:blur-[1px] group-hover:scale-[0.98] transition-all duration-500">
                    ...
                    <div className="absolute inset-0 z-10 flex flex-col justify-end p-2 sm:p-4 bg-linear-to-t from-black/80 via-black/20 to-transparent">
...
### File: components\cabinet\FlavorView.tsx
```
'use client';
...
export default function FlavorView({ dict }: { dict?: any }) {
    ...
    if (!profile && !isAnalyzing) {
        return (
            ...
            >
                ...
                <h2 className="text-2xl font-bold mb-3">{dict.flavor?.empty || (isEn ? "No taste analysis yet" : "아직 분석된 취향이 없습니다")}</h2>
                <p className="text-neutral-400 max-w-md mb-8">
                    {dict.flavor?.emptyDesc || (isEn
                        ? "AI analyzes your taste DNA based on spirits in your cabinet and your reviews."
                        : "보관함에 있는 술과 남기신 리뷰를 바탕으로 AI가 당신의 미각 DNA를 분석해드립니다.")}
                </p>
                <button
                ...
                >
                    <Sparkles className="w-5 h-5" /> {dict.flavor?.start || (isEn ? "Start Analysis" : "내 취향 분석 시작하기")}
                </button>
            </motion.div>
        );
    }
    ...
    if (isAnalyzing) {
        return (
            <div className="flex flex-col items-center justify-center py-32 space-y-6">
                ...
                <p className="text-lg font-medium animate-pulse">{dict.flavor?.analyzing || (isEn ? "Finding your perfect taste match..." : "당신에게 딱 맞는 취향을 찾는 중...")}</p>
            </div>
        );
    }
        ...
        if (usage.remaining > 0) {
            return (
                ...
                >
                    ...
                    {dict.flavor?.reanalyze ? `${dict.flavor.reanalyze} (Today: ${usage.remaining})` : (isEn ? `Re-analyze (Remaining: ${usage.remaining})` : `취향 재분석 (오늘 남은 횟수: ${usage.remaining}회)`)}
                </button>
            );
        } else {
            return (
                <div className="mt-6 w-full py-4 bg-neutral-800/50 border border-neutral-800 text-neutral-500 font-medium rounded-xl text-center text-sm">
                    {dict.flavor?.limitReached || (isEn ? "Daily limit reached😢 (3/day)" : "분석 횟수가 소진되었습니다😢 (일 3회)")}<br />
                    <span className="text-xs opacity-70">{dict.flavor?.seeYouTomorrow || (isEn ? "See you tomorrow!" : "내일 다시 만나요!")}</span>
                </div>
            );
        }
    };
        ...
        >
            ...
            <div className="grid grid-cols-2 gap-4">
                ...
                >
                    ...
                    {dict.flavor?.saveImage || (isEn ? "Save Image" : "이미지 저장")}
                </button>
                <button
                ...
                >
                    ...
                    {dict.flavor?.share || (isEn ? "Share" : "친구에게 공유")}
                </button>
            </div>
...
### File: components\cabinet\GuestOverlay.tsx
```
'use client';
...
### File: components\cabinet\MindMap.tsx
```
'use client';
...
### File: components\cabinet\MyCabinet.tsx
```
import { useState } from "react";
import { motion } from "framer-motion";
import { Spirit } from "@/lib/utils/flavor-engine";
...
### File: components\cabinet\PreferenceExploration.tsx
```
'use client';
...
### File: components\cabinet\ReviewModal.tsx
```
//미사용
'use client';
...
### File: components\cabinet\SearchSpiritModal.tsx
```
import { useState, useEffect } from "react";
import { motion, AnimatePresence } from "framer-motion";
...
export default function SearchSpiritModal({ isOpen, onClose, onSuccess, existingIds, dict, lang }: SearchSpiritModalProps) {
    ...
    const handleAdd = async (item: any, isWishlist: boolean) => {
        if (!user) {
            setToastMessage(isEn ? 'Login required.' : '로그인이 필요합니다.');
            setToastVariant('error');
            setShowToast(true);
            return;
        }
        ...
        try {
            ...
            onSuccess(isEn
                ? (isWishlist ? "Added to wishlist! 🔖" : "Saved to cabinet! 🥃")
                : (isWishlist ? "위시리스트에 담겼습니다! 🔖" : "성공적으로 술장에 담겼습니다! 🥃")
            );
        } catch (e) {
            console.error(e);
            setToastMessage(isEn ? 'An error occurred.' : '오류가 발생했습니다.');
            setToastVariant('error');
            setShowToast(true);
        } finally {
                ...
                >
                    ...
                    <div className="p-6 sm:p-8 border-b border-white/5 relative z-10">
                        <div className="flex justify-between items-center mb-6">
                            <div>
                                <h2 className="text-2xl font-black text-white tracking-tight">{isEn ? 'Add Collection' : '콜렉션 추가'}</h2>
                    ...
                    <div className="flex-1 overflow-y-auto custom-scrollbar p-4 sm:p-6 space-y-2 relative z-10 min-h-[300px]">
                        ...
                        <div className="space-y-3">
                            {results.map(item => {
                                ...
                                const displayName = isEn ? (item.en || item.n) : item.n;
...
### File: components\cabinet\TastePublicReport.tsx
```
'use client';
...
### File: components\cabinet\TasteRadar.tsx
```
'use client';
...
### File: components\common\AdSlot.tsx
```
'use client';
...
### File: components\contents\SpiritGuideLayout.tsx
```
'use client';
import { useState } from 'react'
import Link from 'next/link'
...
### File: components\home\DailyPick.tsx
```
'use client';
...
### File: components\home\NewsSection.tsx
```
import Link from 'next/link';
...
### File: components\home\WikiSnippetSection.tsx
```
'use client';
...
export default function ProfileClient({ lang, dict }: ProfileClientProps) {
    ...
    if (loading) return (
        <div className="flex flex-col items-center justify-center py-20 text-muted-foreground animate-pulse">
            ...
            <p className="font-bold">{isEn ? "Checking account details..." : "계정 정보를 확인하는 중입니다..."}</p>
        </div>
    );
                            ...
                            <div className="relative mb-8 w-full">
                                <div className="grid grid-cols-3 gap-4 w-full">
                                    <Link href={`/${lang}/me/reviews`} className="bg-secondary/30 p-4 rounded-2xl flex flex-col items-center hover:bg-secondary/50 transition-colors">
                                        ...
                                        <span className="text-xs text-muted-foreground">{dict?.reviews || (isEn ? "Reviews" : "리뷰")}</span>
                                    </Link>
                                    <div className="bg-secondary/30 p-4 rounded-2xl flex flex-col items-center">
                                        ...
                                        <span className="text-xs text-muted-foreground">{dict?.likes || (isEn ? "Likes" : "좋아요")}</span>
                                    </div>
...
### File: components\spirits\ModificationRequestButton.tsx
```
'use client';
...
### File: components\ui\CabinetSelectionModal.tsx
```
'use client';
...
### File: components\ui\ChatSommelier.tsx
```
'use client';
...
### File: components\ui\ExploreCard.tsx
```
'use client';
...
### File: components\ui\ExploreSkeleton.tsx
```
'use client';
...
### File: components\ui\GoogleAd.tsx
```
'use client';
...
### File: components\ui\LiveReviews.tsx
```
'use client';
...
### File: components\ui\PwaInstallPrompt.tsx
```
'use client';
...
### File: components\ui\RandomBackground.tsx
```
'use client';
...
### File: components\ui\ReviewSection.tsx
```
'use client';
...
### File: components\ui\SpiritCard.tsx
```
'use client';
...
### File: components\ui\SpiritDetailModal.tsx
```
'use client';
...
### File: components\ui\StickyFooterAd.tsx
```
'use client';
...
### File: components\ui\TagMultiSelect.tsx
```
'use client';
