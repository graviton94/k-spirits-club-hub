...
function hasValidNoseTags(tags: unknown): boolean {
...
async function main() {
  ...
  console.log('[sync-publish-by-nose-tags] Loading spirits...');
  const spirits = await listAllSpirits();
  console.log(`[sync-publish-by-nose-tags] Loaded ${spirits.length} spirits`);
  ...
  for (const spirit of spirits) {
    ...
    inspected += 1;
    const noseTags = Array.isArray(spirit.noseTags)
      ? spirit.noseTags.filter((t) => typeof t === 'string' && t.trim().length > 0)
      : [];

    const hasNose = hasValidNoseTags(noseTags);
    if (hasNose) withNose += 1;
    else withoutNose += 1;

    const targetStatus: 'PUBLISHED' | 'DRAFT' = hasNose ? 'PUBLISHED' : 'DRAFT';
    const targetIsPublished = hasNose;

    const statusMatches = (spirit.status ?? null) === targetStatus;
    const publishMatches = (spirit.isPublished ?? false) === targetIsPublished;

    if (statusMatches && publishMatches) {
      unchanged += 1;
      continue;
    }
    ...
    if (updated % 100 === 0) {
      console.log(`[sync-publish-by-nose-tags] Updated ${updated} records so far...`);
    }
  }

  const backupPath = path.join(outDir, `spirit-publish-sync-backup-${timestamp}.json`);
  await fs.writeFile(
    backupPath,
    JSON.stringify(
      {
  ...
  console.log('[sync-publish-by-nose-tags] Done');
  console.log(`- inspected: ${inspected}`);
  console.log(`- updated: ${updated}`);
  console.log(`- unchanged: ${unchanged}`);
  console.log(`- withNose: ${withNose}`);
  console.log(`- withoutNose: ${withoutNose}`);
  console.log(`- backup: ${backupPath}`);
}

main().catch((error) => {
  console.error('[sync-publish-by-nose-tags] Failed:', error);
  process.exit(1);
});
