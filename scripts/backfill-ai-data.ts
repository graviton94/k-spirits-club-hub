...
function mapAISpiritToSQL(current: any, aiData: any) {
  ...
  return {
    ...
    nameEn: aiData.name_en || current.nameEn || null,
    categoryEn: aiData.category_en || current.categoryEn || null,
    ...
    descriptionKo: aiData.description_ko || current.descriptionKo || null,
    descriptionEn: aiData.description_en || current.descriptionEn || null,
    pairingGuideKo: aiData.pairing_guide_ko || current.pairingGuideKo || null,
    pairingGuideEn: aiData.pairing_guide_en || current.pairingGuideEn || null,
    noseTags: aiData.nose_tags || current.noseTags || [],
    palateTags: aiData.palate_tags || current.palateTags || [],
    finishTags: aiData.finish_tags || current.finishTags || [],
    tastingNote: aiData.tasting_note || current.tastingNote || null,
    status: status,
    ...
    for (let i = 0; i < targets.length; i++) {
      ...
      try {
        ...
        const aiData = await enrichSpiritWithAI({
          ...
          name_en: current.nameEn || undefined,
        });
